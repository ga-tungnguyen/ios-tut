...
class TDRadioButton: UIButton {
    ...
    @IBInspectable var padding: CGFloat = 5.0 {
        didSet {
            guard padding == padding else {return} // thực ra là Radius
            if padding != oldValue {
                self.drawCircle()
            }
        }
    ...
    @IBInspectable var lineWidth: CGFloat = 1 {
        didSet {
            guard lineWidth == lineWidth else {return} // chiều rộng của vòng tròn
    ...
    private var circleLayer = CAShapeLayer() // dùng CAShapeLayer để vẽ
